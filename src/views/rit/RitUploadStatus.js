...
const getBadge = (status) => {
  switch (status) {
    ...
    case "Uploaded":
      return "success";
    case "Failed to upload for error at first row":
      return "danger";
    default:
      return "primary";
  }
...
const RitUploadStatusList = () => {
  ...
  const customizeRitName = (name) => {
    // if (name.length > 15) {
    //   let first = name.substr(0, 18);
    //   let last = name.substr(18, name.length);
    //   return first + "\n" + last;
    // }
    return name;
  };
            ...
            <CCardBody>
              <CDataTable
                ...
                scopedSlots={{
                  ...
                  rit: (item) => <td>{customizeRitName(item.rit)}</td>,
                  // financialInstitute: (item) => (
                  //   <td>{item.financial_institute?.toString()}</td>
                  // ),
                  prepared_by: (item) => (
                    <td>{item.prepared_by?.toString()}</td>
                  ),
                  base_date: (item) => <td>{customizeDate(item.base_date)}</td>,
                  upload_time: (item) => (
                    <td>{customizeDate(item.upload_time)}</td>
                  ),
                  status: (item) => (
                    <td>
                    ...
                    return (
                      <CCollapse show={details.includes(index)}>
                        <CCardBody>
                          ...
                          <p className="text-muted">
                            <b>Bank:</b> {item.financial_institute?.toString()}
                          </p>
                          <p className="text-muted">
                            <b>Branch:</b> {item.branch?.toString()}
                          </p>
                          <p className="text-muted">
                            <b>Prepared By:</b> {item.prepared_by}
                          </p>
                          <p className="text-muted">
                            <b>Base Date:</b> {customizeDate(item.base_date)}
                          </p>
                          <p className="text-muted">
                            <b>Upload Time:</b>{" "}
                            {customizeDate(item.upload_date)}
                          </p>
                          <p className="text-muted">
                            <b>Mobile:</b> {item.phone}
                          </p>
                          <p className="text-muted">
                            <b>IP Address:</b> {item.ip}
                          </p>
