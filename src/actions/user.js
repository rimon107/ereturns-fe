...
export const updateUserStatusAfterLogin =
...
// UPDATE USER STATUS AFTER LOGIN
export const updateUserStatusAfterLogout =
  (id) => async (dispatch, getState) => {
...
export const checkUserCanBeCreated =
  (fi_id, branch_id) => async (dispatch, getState) => {
    try {
      const res = api.get(
        `registration/check-limit/?fi_id=${fi_id}&branch_id=${branch_id}`
      );
      return res;
    } catch (err) {
      return null;
