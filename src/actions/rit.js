...
export const loadRitFeatures =
  ...
  async (dispatch, getState) => {
    try {
      ...
      if (frequency_id) {
        res = await api.get(
          `rit/features/?frequency_id=${frequency_id}`,
          requestOptions(getState)
        );
      } else {
...
// LOAD Rit Departments
export const loadRitFilesByDepartments =
  (department_id) => async (dispatch, getState) => {
    try {
      const res = await api.get(
        `rit/files/?department_id=${department_id}`,
        requestOptions(getState)
      );
      dispatch({
...
export const loadRitUploadStatusReportByBaseDate =
  (rit, fi, base_date, date_from, date_to) => async (dispatch, getState) => {
    try {
      const res = await api.get(
        `rit/report/upload-status/base-date-wise/?rit_id=${rit}&fi_id=${fi}&base_date=${base_date}&date_from=${date_from}&date_to=${date_to}`,
        requestOptions(getState)
      );
      dispatch({
...
export const loadRitUploadStatusReportByFI =
  (rit, fi, date_from) => async (dispatch, getState) => {
    try {
      const res = await api.get(
        `rit/report/upload-status/bank-wise/?rit_id=${rit}&fi_id=${fi}&date_from=${date_from}`,
        requestOptions(getState)
      );
      dispatch({
        type: RIT_REPORT_DATA_LOAD,
...
export const loadRitNonReportingBranchByFI =
  (rit, fi, base_date) => async (dispatch, getState) => {
    try {
      const res = await api.get(
        `rit/report/upload-status/non-reporting-branch/?rit_id=${rit}&fi_id=${fi}&base_date=${base_date}`,
        requestOptions(getState)
      );
      dispatch({
        type: RIT_REPORT_DATA_LOAD,
...
// LOAD Rit Files
export const loadRitDetailsByCode = (id) => async (dispatch, getState) => {
  try {
    const res = await api.get(
      `rit/files/?rit_id=${id}`,
      requestOptions(getState)
...
export const loadRitValidationDataByCode =
  (code) => async (dispatch, getState) => {
    try {
      const res = await api.get(
        `rit/validation/?code=${code}`,
        requestOptions(getState)
      );
      dispatch({
